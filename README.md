...
### 2. Run the pipeline
```bash
# Use included dataset (auto-detected)
python cardiovascular_biomarker_ml.py

# Or specify your own Kaggle download
python cardiovascular_biomarker_ml.py Heart_Disease_UCI.csv
```

### 3. View results
All figures and CSV reports are saved to `results/`.
...
### `statistical_analysis.csv`
- Per-feature p-values (Mann-Whitney U or Chi²)
- Point-Biserial correlation coefficient (effect size)
- Mean values for Disease vs No-Disease groups

### `lasso_coefficients.csv`
- Which features LASSO kept vs. zeroed out
- Coefficient sign = direction of risk (↑ or ↓)
- Optimal regularisation strength (alpha)

### `feature_importance.csv`
- 7 independent importance scores per feature
- Composite score (mean of normalised methods)
- Use this to identify the most robust biomarkers

### `model_results.csv`
- CV accuracy, AUC, F1 ± std for all models
- Hold-out test Accuracy, AUC, F1, MCC
- Sorted by Test AUC (best first)

---

## Interpreting SHAP Figures (Fig 14)

| Plot | What it shows |
|------|--------------|
| **Beeswarm** | Each dot = one patient. X-axis = SHAP value (impact). Color = feature value (red=high, blue=low) |
| **Bar** | Mean absolute SHAP per feature — global importance ranking |
| **Waterfall** | Single patient breakdown — how each feature pushed the prediction up/down |
| **Dependence** | Relationship between feature value and its SHAP value (interaction effects shown via color) |
